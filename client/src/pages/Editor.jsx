...
const WebsiteEditor = () => {
  ...
  // useEffect is used to render dynamic HTML code inside an iframe whenever website changes
  useEffect(()=>{
            ...
            <>
                    ...
                    {messages.map((m, i)=>(
                        ...
                        className={`max-w-[85%] ${m.role === "user" ? "mr-auto" : "ml-auto"}`}>
                            <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed 
                                ${m.role === "user" ? "bg-white text-black" : 
                ...
                <div className='p-3 border-t border-white/10'>
                    <div className='flex gap-2'>
                        ...
                        className='flex-1 resize-none rounded-2xl px-4 py-3 bg-white/5 border border-white/10
                        text-sm outline-none'
                        value={prompt}
                        onChange={(e)=>setPrompt(e.target.value)} />
                        <button className='px-4 py-3 rounded-2xl bg-white text-black cursor-pointer' disabled={updateLoading}
            ...
            <div className='h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80'>
                ...
                <div className='flex gap-2'>
                    {website.deployed ? "" : 
                        ...
                        className='flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r from-indigo-500 to-purple-500 text-sm
                    ...
                    <button className='p-2 cursor-pointer' onClick={()=>setShowCode(true)}>
                        <Code2 size={18} />
                    </button>
                    <button className='p-2 cursor-pointer' onClick={()=>setShowFullPreview(true)}><Monitor size={18} /></button>
                </div>
            </div>
        ...
        <AnimatePresence>
            {showFullPreview && (
                <motion.div className='fixed inset-0 z-[9999] bg-black'>
                    ...
                    sandbox='allow-scripts allow-same-origin allow-forms' />
                    <button className='absolute top-4 right-4 p-2 bg-black/70 rounded-lg cursor-pointer' 
